...
int LSM9DS1::LSM9DS1M::lsm9ds1m_init() {
  ...
  result = writeReg(LSM9DS1M_CTRL_REG1_M,
                    BITS_TEMP_COMP | BITS_OM_HIGH | BITS_ODR_M_80HZ);
...
int LSM9DS1::start() {
  ...
  if (sensor_id != LSM9DS1_WHO_AM_I_ACC_GYRO) {
    DF_LOG_ERR("LSM9DS1 sensor WHOAMI wrong: 0x%X, should be: 0x%X", sensor_id,
               LSM9DS1_WHO_AM_I_ACC_GYRO);
    result = -1;
    goto exit;
  }
...
int LSM9DS1::LSM9DS1M::start() {
  ...
  if (sensor_id != LSM9DS1_WHO_AM_I_MAG) {
    DF_LOG_ERR("LSM9DS1M sensor WHOAMI wrong: 0x%X, should be: 0x%X", sensor_id,
               LSM9DS1_WHO_AM_I_MAG);
    result = -1;
    goto exit;
  }
...
void LSM9DS1::_measure() {
  ...
  if (_mag_enabled) {
    ...
    DF_LOG_DEBUG("mag x: %d, y: %d, z: %d", report.mag_x, report.mag_y,
                 report.mag_z);
  ...
  DF_LOG_DEBUG("accel x: %d, y: %d, z: %d", report.accel_x, report.accel_y,
               report.accel_z);
  DF_LOG_DEBUG("gyro x: %d, y: %d, z: %d", report.gyro_x, report.gyro_y,
               report.gyro_z);
  ...
  if (++m_sensor_data.read_counter % (1000000 / 1000) == 0) {
    ...
    DF_LOG_INFO(
        "IMU: accel: [%f, %f, %f] m/s^2", (double)m_sensor_data.accel_m_s2_x,
        (double)m_sensor_data.accel_m_s2_y, (double)m_sensor_data.accel_m_s2_z);
    DF_LOG_INFO(
        "     gyro:  [%f, %f, %f] rad/s", (double)m_sensor_data.gyro_rad_s_x,
        (double)m_sensor_data.gyro_rad_s_y, (double)m_sensor_data.gyro_rad_s_z);

    if (_mag_enabled) {
      DF_LOG_INFO("     mag:  [%f, %f, %f] ga", (double)m_sensor_data.mag_ga_x,
